...
public class ServerThread extends Thread{
    ...
    boolean TokenServerAlive;

    public ServerThread(Socket socket){
        this.clientSocket = socket;
        TokenServerAlive = true;
    }

    public void run(){
        InputStream clientInputStream = null;
        OutputStream clientOutputStream = null;
        while (TokenServerAlive) {
            try {
                if(VideoCallServer.TokenLock.tryLock(800, TimeUnit.MICROSECONDS)) {
                    ...
                    VideoCallServer.TokenLock.unlock();
                    TokenServerAlive = false;
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
